...
def assert_value_is_valid_type(check_value: Any, 
    ...
    assert_eval: bool = isinstance(check_value, check_type_list)
    assert_error: str = f"Value {check_value=} is not expected type ({check_type_list=})."
...
def assert_value_in_range(check_value: int | float,
                          check_range: range,
                          raise_error: bool = True
                          ) -> bool:
    """
